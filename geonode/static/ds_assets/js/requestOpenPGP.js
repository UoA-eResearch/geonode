...
//public keys in 'public_keys.js' file
...
{   
  ...
  //17-June-2023..  ideally all the users will be from the database and captured via javascript
  const publicKeysArmored = [publicKey_my, publicKey_amiria]; 
  ...
  */
  ...
  //Not needed    document.getElementById('view_resourcebase_users').onclick = function() {
  //Not needed    var radios = document.getElementsByName('view_resourcebase_users');
  //console.log('#### Here: ' +  $("#view_resourcebase_users").val() );
  var array = $("#view_resourcebase_users").val();
  //to_encrypt_users_array = array; //update gloabal variable in 
  for (var val of array)  {
    //showEncryptionWithUsersPublicKeySuccessMessage('Dataset has been encrypted with user ' + val + '\'s public key'); 
    showFileUploadSuccessMessage('Dataset has been encrypted with user ' + val + '\'s public key','info'); // as a Layer and Masked Encrypted Layer being uploaded as document');                              
    console.log('user ' + val);                  
  }  
