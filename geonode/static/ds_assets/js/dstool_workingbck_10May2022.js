...
async function multiLevelEncrypt(maskedData_string, maskedMedData_string){ 
    ...
    console.log('Level 1 changes begins');
    console.log("original.geojson" + originalmap_jsonString); 
    ...
    /*zipLevel1.file("Metadata.txt", //above has values here
            "Maybe include masking paramneters" +
 //           "Masking parameters of Level 2 \nRandDist: " + randDist + "\nRandAngle: " + randAngle +
    ...
    */
    ...
    try { 
        encryptedZipFileData_Level1 = await zipCallEncrypt(zipLevel1, 1);  // Await for the first function to complete
    }
...
/*  07-May-2022...Commented these two levels, as with OpenPGP, we only encrypt at one level.     
    ...
    try {
         encryptedZipFileData_Level2 = await zipCallEncrypt(zipLevel2, 2); //second parameter is the level 
    }
    catch (error) { console.error(error);  } 
    ...
    try {
        encryptedZipFileData_Level3 = await zipCallEncrypt(zipLevel3, 3);
    }
    catch (error) { console.error(error); }
*/
    endTime = new Date();
...
async function zipCallEncrypt(zipFile, level) {
    ...
    jsZip.loadAsync(v_objFile).then(function (zip) {
        Object.keys(zip.files).forEach(function (filename){
                ...
                zip.files[filename].async('string').then(function (fileData){
                    ...
                    {
                        console.log("Calling recursiveDecrypt function again");
...
function display_hash() {
    console.log("displaying hash2: " + hash_value); 
    //if(hash_value === null)
        ...
        fetch(url, requestOptions)
            ...
            .then(result => {
              ...
              //link.href = myPath;              
        			...
        			var link = "link";
        			var result = text + link.link(myPath);
...
function upload_layer(masked_reprojected){ //maskedGeoJSON_string) {   //filename = "sample.geojson",
  ...
  if(formatted_masked_reprojected === null) //formatted in xyz.js, line 379
