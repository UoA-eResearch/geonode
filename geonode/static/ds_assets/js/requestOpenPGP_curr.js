...
function openpgp_encryptTextFile(){
  ...
  var file = new File([blob], "decrypted.txt", { type: "text/plain", });
  ...
  fileReader.onload = function() {
    ...
    //encryptedZipFile = OpenPGPEncryptData(false, file, downloadFile, level);
  }
  ...
  return new Promise((resolve, reject)  => {
      ...
      zipFile.generateAsync({type: "blob"})  //https://stuk.github.io/jszip/
          .then(function (content) {  
              ...
              //actually, according to documentation, this gets called almost as soon as fileReader has been created
...
//    console.log('str: ' + str);    
  const message = await openpgp.createMessage({ text: plainTextString }); //plainTextString //str //plaintextbytes
  console.log('plainTextString: ' + plainTextString);
...
/*
  //08-June-2022 : https://github.com/adcreare/nodejs-demos/blob/master/openpgp.js/file-encryption-decryption-example-es-2017.js
  const publicKeys = await Promise.all(publicKeysArmored.map(armoredKey => openpgp.readKey({ armoredKey })));
  const fileForOpenpgpjs = new Uint8Array(file_contents);
    const options = {
        data: fileForOpenpgpjs,
        publicKeys: publicKeys, //openpgpPublicKey.keys,
        armor: false
    };
...
*/
  ...
  var blob3 = new Blob([encrypted],{ type: 'text/plain;charset=utf-8' }); //{type: 'text/plain'} );  //cipherbytes  
  //var blob3 = new Blob([encryptionResponse],{type: 'text/plain'} );  //cipherbytes  
...
//$$     console.log('Decrypted data: ' + plaintext.data);
    ...
    var blob = new Blob([plaintext.data],{type: 'text/plain'} ); // save as Blob 
...
*/
  ...
  */
  const fileForOpenpgpjs = new Uint8Array(cipherTextInFile);
    const options = {
        privateKey: privKeyObj, //openpgpPrivateKeyObject,
        message: openpgp.message.read(cipherTextInFile),
        format: 'binary'
    };
    const decryptionResponse = await openpgp.decrypt(options);
    const decryptedFile = decryptionResponse.data;
    var blob = new Blob([decryptionResponse],{type: 'text/plain'} ); // save as Blob 
    saveAs(blob, fileName2);
