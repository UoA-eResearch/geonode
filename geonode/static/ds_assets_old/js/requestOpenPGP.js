...
async function callOpenPGPEncrypt()  { //v_ciphertextInFile, levelToDecrypt, currentLevel){ //v_objFile
  ...
  zipLevel1.file("original.geojson", originalmap_jsonString);  //add original map file in geojson format to zip file
  zipLevel1.file(fileName + ".zip", fileInput.files[0]);       //add the original uploaded shapefile (the original file loaded by user)
  ...
  zipLevel1.generateAsync({type: "blob", compression: "DEFLATE"}, )  //https://stuk.github.io/jszip/
...
//$$$ key1 to key2
  const publicKeysArmored = [publicKey1, publicKey2];
